...
const ROUTE_ARG_KEYS = Object.freeze(['text', 'actor_id', 'object_refs', 'intent', 'context']);
const HUMAN_COMMIT_INTENTS = new Set([
  'assign',
  'give-access',
  'revoke',
  'revoke-access',
...
export function buildIntentRouteArgs({
  text,
  actor_id,
  object_refs,
  focused_object_ref,
  ...
  const refs = Array.isArray(object_refs)
    ? object_refs.map((value) => String(value || '').trim()).filter(Boolean)
    : [];
  const context = {};
  const focused = String(focused_object_ref || '').trim();
  const conversation = String(conversation_id || '').trim();
  const session = String(code_session_id || '').trim();
  if (focused) context.focused_object_ref = focused;
  if (conversation) context.conversation_id = conversation;
  if (session) context.code_session_id = session;
  if (Array.isArray(known_actors) && known_actors.length) context.known_actors = known_actors;
  const args = pickAllowedArgs(ROUTE_ARG_KEYS, {
    text: cleanText,
    actor_id: String(actor_id || '').trim() || undefined,
    object_refs: refs.length ? refs : undefined,
...
function proposalArgsFrom(payload = {}, proposal = findProposal(payload)) {
  ...
  const tool = proposal?.mcp_tool || proposal?.tool_id || payload.mcp_tool || payload.tool_id || null;
...
export function compileIntentProposalCard(routeResult = {}) {
  ...
  const proposalTool = proposal?.mcp_tool || proposal?.tool_id || routeResult.mcp_tool || null;
  ...
  return {
    ok,
    intent,
    proposal: hasProposal ? { mcp_tool: proposalTool, args: proposalArgs } : null,
...
export function compileDispatchCommitCard(taskRun = {}) {
  ...
  const receipt_kind = findReceiptKind(taskRun)
    || taskRun.context_resolution
    || taskRun.route_receipt?.context_resolution
    || null;
  const lines = [
